  ...
  const getAnimation = useCallback(() => {
    if (isFlipping) {
      if (coinChoice < HEADS) return { animation: "flipHeads 5s forwards" };
        ...
        return { animation: "flipTails 5s forwards" };
  ...
  return (
    <div className="Coin">
      ...
      {!isFlipping && coinHistory.length > 0 ? (
        <div className="Winnings">{`Earned ${formatNumberWithCommas(
          winnings
            ...
            Net gain ${formatNumberWithCommas(winnings - bet)} mana.`}</div>
      ) : (
      ...
      <div
        ...
        style={
          winnings >= bet * 2 && mintCheers && !isFlipping
      ...
      >
        <img
          src={winnings >= bet * 35 ? scarletArcade2 : scarletArcade1}
          alt=""
        />
      </div>
    </div>
